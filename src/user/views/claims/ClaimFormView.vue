...
import WizardStepBar from '@user/components/claim/WizardStepBar.vue'
import ConsentOverlay from '@user/components/claim/ConsentOverlay.vue'
import ClaimFieldInput from '@user/components/claim/ClaimFieldInput.vue'
...
// ===== 자동채움: Step 3 (회원 정보와 동일) =====
const CONTRACTOR_USER_MAP: Record<string, string> = {
  contractor_name: 'name',
  contractor_phone: 'phone',
  contractor_email: 'email',
  contractor_resident_number: 'resident_number',
}
...
function handleAutoFillFromUser() {
  ...
  if (autoFillFromUser.value && customer) {
    currentStepFields.value.forEach(field => {
      const customerKey = CONTRACTOR_USER_MAP[field.field_name.toLowerCase()]
      if (customerKey) {
        const val = customer[customerKey as keyof typeof customer]
        if (val && typeof val === 'string') {
          claimStore.setFieldValue(field.form_field_id, val)
        }
      }
    })
  } else {
    currentStepFields.value.forEach(field => {
      if (CONTRACTOR_USER_MAP[field.field_name.toLowerCase()]) {
...
// ===== 자동채움: Step 4 (계약자와 동일) =====
const INSURED_CONTRACTOR_MAP: Record<string, string> = {
  insured_name: 'contractor_name',
  insured_phone: 'contractor_phone',
  insured_email: 'contractor_email',
  insured_resident_number: 'contractor_resident_number',
}

const BENEFICIARY_CONTRACTOR_MAP: Record<string, string> = {
  beneficiary_name: 'contractor_name',
  beneficiary_phone: 'contractor_phone',
  beneficiary_email: 'contractor_email',
  beneficiary_resident_number: 'contractor_resident_number',
}

function getContractorFieldValue(contractorFieldName: string): string {
  const contractorField = allFieldsSorted.value.find(
    f => f.field_name.toLowerCase() === contractorFieldName
  )
  if (!contractorField) return ''
  return claimStore.fieldValues[contractorField.form_field_id] || ''
}

function handleAutoFillInsured() {
  if (autoFillInsuredFromContractor.value) {
    insuredStepFields.value.forEach(field => {
      const contractorName = INSURED_CONTRACTOR_MAP[field.field_name.toLowerCase()]
      if (contractorName) {
        const val = getContractorFieldValue(contractorName)
        if (val) claimStore.setFieldValue(field.form_field_id, val)
      }
    })
  } else {
    insuredStepFields.value.forEach(field => {
      if (INSURED_CONTRACTOR_MAP[field.field_name.toLowerCase()]) {
        claimStore.setFieldValue(field.form_field_id, '')
      }
    })
  }
...
function handleAutoFillBeneficiary() {
  if (autoFillBeneficiaryFromContractor.value) {
    beneficiaryStepFields.value.forEach(field => {
      const contractorName = BENEFICIARY_CONTRACTOR_MAP[field.field_name.toLowerCase()]
      if (contractorName) {
        const val = getContractorFieldValue(contractorName)
        if (val) claimStore.setFieldValue(field.form_field_id, val)
      }
    ...
    beneficiaryStepFields.value.forEach(field => {
      if (BENEFICIARY_CONTRACTOR_MAP[field.field_name.toLowerCase()]) {
        claimStore.setFieldValue(field.form_field_id, '')
      }
    })
  }
...
function handleAutoFillStep4() {
  // Step 3 (계약자) 필드값을 Step 4 필드에 같은 field_type 기준으로 복사
  const step3Fields = wizardDisplayFields.value.filter(f => getFieldWizardStep(f) === 3)
  if (autoFillInsuredFromContractor.value) {
    currentStepFields.value.forEach(field => {
      const matching = step3Fields.find(sf => sf.field_type === field.field_type)
      if (matching) {
        const val = claimStore.fieldValues[matching.form_field_id]
        if (val) claimStore.setFieldValue(field.form_field_id, val)
      }
    })
  } else {

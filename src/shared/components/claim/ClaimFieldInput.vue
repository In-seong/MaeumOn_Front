  ...
  <div>
    ...
    <div v-else-if="field.field_type === 'resident_number'" class="flex items-center gap-2 w-full min-w-0">
      <input
        ...
        class="flex-1 min-w-0 px-4 py-3 bg-white rounded-[12px] text-[14px] text-[#333] text-center outline-none border border-[#E8E8E8] focus:border-[#FF7B22] transition-colors placeholder-[#B0B0B0]"
      />
      ...
      <div class="flex-1 min-w-0 relative">
        <input
          ...
          maxlength="7"
          ref="residentBackRef"
          class="w-full px-4 py-3 pr-10 bg-white rounded-[12px] text-[14px] text-[#333] text-center outline-none border border-[#E8E8E8] focus:border-[#FF7B22] transition-colors placeholder-[#B0B0B0]"
        />
        <button
          type="button"
    ...
    <!-- 셀렉트 (드롭다운) -->
    <select
      v-else-if="field.field_type === 'select' && field.field_options?.choices"
      :value="modelValue"

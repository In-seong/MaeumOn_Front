<template>
  <div class="p-6">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-[20px] font-bold text-[#333]">배분 관리</h1>
      <span class="text-[13px] text-[#888]">
        총 <span class="font-semibold text-[#FF7B22]">{{ activeTab === 'db' ? dbStore.total + dbStore.claimAssignments.length : corpStore.total }}</span>건
      </span>
    </div>

    <!-- 탭 -->
    <div class="flex gap-1 mb-6 bg-[#F5F5F5] rounded-[12px] p-1 max-w-[300px]">
      <button
        v-for="tab in tabs"
        :key="tab.key"
        :class="[
    ...
    <template v-else-if="activeTab === 'db'">
      <!-- 청구 배정 테이블 -->
      <div v-if="dbStore.claimAssignments.length > 0" class="mb-6">
        ...
        <div class="bg-white rounded-[16px] shadow-[0_0_10px_rgba(0,0,0,0.06)] overflow-x-auto">
          <table class="min-w-full divide-y divide-[#E8E8E8]">
            ...
            <tbody class="bg-white divide-y divide-[#F0F0F0]">
              <tr
                v-for="(item, index) in dbStore.claimAssignments"
      ...
      <!-- DB 배분 테이블 -->
      <div>
        <h2 v-if="dbStore.claimAssignments.length > 0" class="text-[16px] font-semibold text-[#333] mb-3">고객 배분</h2>
        <div class="bg-white rounded-[16px] shadow-[0_0_10px_rgba(0,0,0,0.06)] overflow-x-auto">
          <table class="min-w-full divide-y divide-[#E8E8E8]">
            <thead class="bg-[#FAFAFA]">
              <tr>
            ...
            <tbody class="bg-white divide-y divide-[#F0F0F0]">
              <tr
                v-for="(item, index) in dbStore.distributions"
                :key="item.assignment_id"
                class="hover:bg-[#FAFAFA] transition-colors"
              >
                <td class="px-6 py-4 whitespace-nowrap text-[14px] text-[#999]">{{ dbRowNum(index) }}</td>
              ...
              <tr v-if="dbStore.distributions.length === 0">
          ...
          <!-- 페이지네이션 -->
          <div v-if="dbStore.lastPage > 1" class="px-6 py-4 border-t border-[#F0F0F0]">
            <div class="flex items-center justify-center gap-1">
              <button
                :disabled="dbStore.currentPage <= 1"
                ...
                @click="goToDbPage(dbStore.currentPage - 1)"
              >
                이전
              </button>
              <button
                v-for="page in dbVisiblePages"
                :key="page"
                :class="[
                  'px-3 py-1.5 rounded-[8px] text-[13px] transition-colors',
                  page === dbStore.currentPage
                    ? 'bg-[#FF7B22] text-white'
                    : 'text-[#555] hover:bg-[#FFF3ED] hover:text-[#FF7B22]',
                ]"
                @click="goToDbPage(page)"
              >
                {{ page }}
              </button>
              <button
                :disabled="dbStore.currentPage >= dbStore.lastPage"
                class="px-3 py-1.5 rounded-[8px] text-[13px] text-[#555] hover:bg-[#FFF3ED] hover:text-[#FF7B22] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                @click="goToDbPage(dbStore.currentPage + 1)"
              >
                다음
    ...
    <!-- 기업배분 탭 -->
    <template v-else-if="activeTab === 'corporate'">
      <div class="bg-white rounded-[16px] shadow-[0_0_10px_rgba(0,0,0,0.06)] overflow-x-auto">
        <table class="min-w-full divide-y divide-[#E8E8E8]">
          <thead class="bg-[#FAFAFA]">
            <tr>
          ...
          <tbody class="bg-white divide-y divide-[#F0F0F0]">
            <tr
              v-for="(item, index) in corpStore.inquiries"
              :key="item.id"
              class="hover:bg-[#FAFAFA] transition-colors"
            >
              <td class="px-6 py-4 whitespace-nowrap text-[14px] text-[#999]">{{ corpRowNum(index) }}</td>
            ...
            <tr v-if="corpStore.inquiries.length === 0">
        ...
        <!-- 페이지네이션 -->
        <div v-if="corpStore.lastPage > 1" class="px-6 py-4 border-t border-[#F0F0F0]">
          <div class="flex items-center justify-center gap-1">
            <button
              :disabled="corpStore.currentPage <= 1"
              class="px-3 py-1.5 rounded-[8px] text-[13px] text-[#555] hover:bg-[#FFF3ED] hover:text-[#FF7B22] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              @click="goToCorpPage(corpStore.currentPage - 1)"
            >
              이전
            </button>
            <button
              v-for="page in corpVisiblePages"
              :key="page"
              :class="[
                'px-3 py-1.5 rounded-[8px] text-[13px] transition-colors',
                page === corpStore.currentPage
                  ? 'bg-[#FF7B22] text-white'
                  : 'text-[#555] hover:bg-[#FFF3ED] hover:text-[#FF7B22]',
              ]"
              @click="goToCorpPage(page)"
            >
              {{ page }}
            </button>
            <button
              :disabled="corpStore.currentPage >= corpStore.lastPage"
              class="px-3 py-1.5 rounded-[8px] text-[13px] text-[#555] hover:bg-[#FFF3ED] hover:text-[#FF7B22] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              @click="goToCorpPage(corpStore.currentPage + 1)"
            >
              다음
      ...
      >
        ...
        <div class="relative bg-white rounded-[16px] p-6 w-full max-w-[460px] z-10 shadow-xl">
          ...
          <div class="flex gap-3 mt-4">
            <button
              class="flex-1 py-2.5 border border-[#E8E8E8] text-[#666] rounded-[12px] text-[14px] font-medium hover:bg-[#F8F8F8] transition-colors"
              @click="closeCorpEditForm"
            >
              취소
            </button>
            <button
              class="flex-1 py-2.5 bg-[#FF7B22] text-white rounded-[12px] text-[14px] font-medium hover:bg-[#E56D1E] transition-colors disabled:opacity-50"
              :disabled="corpProcessing"
...
const isLoading = computed(() =>
  activeTab.value === 'db' ? dbStore.loading : corpStore.loading
)
...
// DB배분 페이지네이션
function goToDbPage(page: number): void {
  if (page < 1 || page > dbStore.lastPage) return
  dbStore.currentPage = page
  dbStore.loadDistributions()
}

function dbRowNum(index: number): number {
  return (dbStore.currentPage - 1) * 15 + index + 1
}

const dbVisiblePages = computed(() => {
  const pages: number[] = []
  const total = dbStore.lastPage
  const current = dbStore.currentPage
...
// 기업배분 페이지네이션
function goToCorpPage(page: number): void {
  if (page < 1 || page > corpStore.lastPage) return
  corpStore.currentPage = page
  corpStore.loadInquiries()
}

function corpRowNum(index: number): number {
  return (corpStore.currentPage - 1) * 15 + index + 1
}

const corpVisiblePages = computed(() => {
  const pages: number[] = []
  const total = corpStore.lastPage
  const current = corpStore.currentPage
  let start = Math.max(1, current - 2)
  let end = Math.min(total, current + 2)
  if (end - start < 4) {
    if (start === 1) end = Math.min(total, start + 4)

<template>
  <div class="p-6">
    <h1 class="text-[20px] font-bold text-[#333] mb-6">일정관리</h1>
    ...
    <div class="grid grid-cols-3 gap-6">
      ...
      <!-- 우측: 선택일 일정 리스트 -->
      <div class="bg-white rounded-xl border border-[#E8E8E8] p-5 flex flex-col">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-[16px] font-semibold text-[#333]">
            {{ formattedSelectedDate }}
          </h2>
          <button
            class="px-3 py-1.5 bg-[#FF7B22] text-white text-[13px] font-medium rounded-lg hover:bg-[#E56D1E] transition-colors"
            @click="openAddModal"
          >
            + 일정 추가
          </button>
        ...
        <!-- Loading -->
        <div v-if="store.loading" class="flex items-center justify-center py-12 flex-1">
          <p class="text-[13px] text-[#BBB]">불러오는 중...</p>
        </div>

        <!-- Schedule List -->
        <div v-else class="flex flex-col gap-2 flex-1 overflow-y-auto">
          <ScheduleEventItem
    ...
    <Teleport to="body">
      <Transition name="fade-modal">
        ...
        >
          ...
          <div class="relative w-full max-w-[480px] bg-white rounded-xl p-6 max-h-[85vh] overflow-y-auto z-10 shadow-xl">
            <div class="flex items-center justify-between mb-5">
              <h3 class="text-[18px] font-bold text-[#333]">
            ...
            <div class="flex flex-col gap-4">
              <!-- 일정 유형 -->
              <div>
                <label class="text-[13px] text-[#888] mb-1.5 block font-medium">유형 *</label>
                <div class="flex flex-wrap gap-2">
                  <button
                    v-for="opt in eventTypeOptions"
                    :key="opt.value"
                    type="button"
                    class="px-3 py-1.5 rounded-full text-[13px] font-medium border transition-all cursor-pointer"
                    :class="form.event_type === opt.value ? 'bg-[#FFF0E5] border-[#FF7B22] text-[#FF7B22]' : 'bg-[#F8F8F8] border-[#E8E8E8] text-[#999] hover:border-[#CCC]'"
              ...
              <div>
                <label class="text-[13px] text-[#888] mb-1.5 block font-medium">고객 (선택)</label>
                <select
                  v-model="form.customer_id"
                  class="w-full bg-[#F8F8F8] rounded-lg px-3 py-2.5 text-[14px] border border-[#E8E8E8] outline-none focus:border-[#FF7B22] transition-colors text-[#333] appearance-none cursor-pointer"
                >
              ...
              <!-- 리마인더 설정 -->
              <div>
                <label class="text-[13px] text-[#888] mb-1.5 block font-medium">사전 알림</label>
                <div class="flex flex-wrap gap-2">
                  <button
                    ...
                    type="button"
                    class="px-3 py-1.5 rounded-full text-[13px] font-medium border transition-all cursor-pointer"
                    :class="form.reminder_days.includes(opt.value) ? 'bg-[#FFF0E5] border-[#FF7B22] text-[#FF7B22]' : 'bg-[#F8F8F8] border-[#E8E8E8] text-[#999] hover:border-[#CCC]'"
              ...
              <button
                class="w-full mt-2 py-3 rounded-lg bg-[#FF7B22] text-white text-[14px] font-semibold hover:bg-[#E56D1E] transition-colors disabled:opacity-50"
                :disabled="saving"
                @click="handleSubmit"
              >

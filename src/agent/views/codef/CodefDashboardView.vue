  ...
  <div class="min-h-screen bg-[#F8F8F8]">
    ...
    <div class="px-4 pt-3 pb-28">
      ...
      <div v-else-if="activeTab === 'medical'">
        ...
        <!-- 고지의무 분류 -->
        <div v-if="store.disclosureCheck?.disclosure_classification" class="bg-white rounded-[14px] border border-[#E8E8E8] p-4 mb-3">
          <div class="flex items-center gap-2 mb-3">
            <span class="w-6 h-6 rounded-full bg-[#FFF3ED] flex items-center justify-center text-[13px]">&#x26A0;&#xFE0F;</span>
            <div>
              <p class="text-[14px] font-semibold text-[#222]">고지의무 분류</p>
              <p class="text-[11px] text-[#999]">진료 데이터 기반 자동 분류</p>
            </div>
          </div>

          <template v-for="(item, idx) in classificationItems" :key="item.key">
            <p v-if="item.period" class="text-[10px] font-bold text-[#999] uppercase tracking-wider mb-1" :class="idx > 0 ? 'mt-2.5 pt-2.5 border-t border-[#F0F0F0]' : ''">
              {{ item.period }}
            </p>
            <div @click="item.found ? toggleClassification(item.key) : undefined"
                 class="flex items-center gap-2 px-2.5 py-2 rounded-[8px] mb-0.5 transition-colors"
                 :class="item.found ? 'bg-[#FFF8F3] cursor-pointer active:bg-[#FFEFE0]' : ''">
              <span class="text-[12px] flex-1 font-medium" :class="item.found ? 'text-[#333]' : 'text-[#BBB]'">
                {{ item.label }}
              </span>
              ...
              <template v-if="item.found">
                <span class="text-[11px] text-[#FF7B22] font-medium min-w-[28px] text-right">{{ item.count }}건</span>
                <svg class="w-3 h-3 text-[#999] shrink-0 transition-transform" :class="expandedClassification === item.key ? 'rotate-90' : ''"
                     viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 18l6-6-6-6"/></svg>
              </template>
            </div>

            <div v-if="expandedClassification === item.key && item.found" class="px-2 pb-2 mt-1">
              <template v-if="item.type === 'simple'">
                <div v-for="rec in getClassificationRecords(item.key)" :key="rec.record_id"
                     class="flex items-center gap-1.5 py-1.5 border-b border-[#F5F5F5] last:border-0 text-[11px]">
                  <span class="text-[#999] w-[70px] shrink-0">{{ rec.treatment_date }}</span>
                  <span class="text-[#333] flex-1 truncate">{{ rec.hospital_name }}</span>
                  <span class="text-[#888] truncate max-w-[90px]">{{ rec.diagnosis_name }}</span>
                </div>
              </template>

              <template v-else-if="item.type === 'grouped'">
                <div v-for="group in getClassificationGroups(item.key)" :key="group.diagnosis_code" class="mb-2 last:mb-0">
                  <div class="flex items-center gap-1.5 py-1.5 bg-[#FFF3ED] rounded px-2">
                    <span class="px-1.5 py-0.5 bg-[#FF7B22] text-white rounded text-[10px] font-medium shrink-0">{{ group.diagnosis_code }}</span>
                    <span class="text-[12px] text-[#333] font-medium flex-1 truncate">{{ group.diagnosis_name }}</span>
                    <span class="text-[12px] text-[#FF7B22] font-bold shrink-0">{{ group.displayValue }}</span>
                  </div>
                  <div v-for="rec in group.records.slice(0, 5)" :key="rec.record_id"
                       class="flex items-center gap-1.5 py-1 pl-3 text-[10px] text-[#999]">
                    <span class="w-[70px] shrink-0">{{ rec.treatment_date }}</span>
                    <span class="flex-1 truncate">{{ rec.hospital_name }}</span>
                  </div>
                  <p v-if="group.records.length > 5" class="text-[10px] text-[#BBB] pl-3 py-0.5">
              ...
              <template v-else-if="item.type === 'critical'">
                <div v-for="(recs, dKey) in getCriticalClassificationRecords()" :key="dKey" class="mb-2 last:mb-0">
                  <p class="text-[11px] font-semibold text-[#E91E63] py-1 px-1">{{ criticalDiseaseLabel(dKey as string) }}</p>
                  <div v-for="rec in recs.slice(0, 5)" :key="rec.record_id"
                       class="flex items-center gap-1.5 py-1 pl-3 text-[10px] text-[#999]">
                    <span class="w-[70px] shrink-0">{{ rec.treatment_date }}</span>
                    <span class="flex-1 truncate">{{ rec.hospital_name }}</span>
                    <span class="truncate max-w-[80px]">{{ rec.diagnosis_name }}</span>
                  </div>
                  <p v-if="recs.length > 5" class="text-[10px] text-[#BBB] pl-3 py-0.5">
                    외 {{ recs.length - 5 }}건 더
                  </p>
                </div>
              </template>
        ...
        <!-- 알릴의무 체크 테이블 -->
        <div v-if="store.disclosureCheck" class="bg-white rounded-[14px] border border-[#E8E8E8] p-4 mb-3">
          <div class="flex items-center gap-2 mb-3">
            <span class="text-[16px]">&#x1F4CB;</span>
            <div>
              <p class="text-[14px] font-semibold text-[#222]">입원/수술 보장 기간</p>
              <p class="text-[11px] text-[#999]">보장 기간을 한눈에 확인하세요.</p>
            </div>
            <div class="ml-auto flex items-center gap-1 text-[11px] text-[#999]">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke-width="2"/><path d="M12 16v-4M12 8h.01" stroke-width="2" stroke-linecap="round"/></svg>
          ...
          <div class="overflow-x-auto -mx-1">
            <table class="w-full text-[11px] border-collapse">
              <thead>
                <tr>
                  <th rowspan="2" class="border border-[#E8E8E8] bg-[#FFF3ED] text-[#FF7B22] px-2 py-1.5 text-center font-semibold whitespace-nowrap align-middle">
                    3개월<br/>이내<span class="text-red-500">*</span>
                  </th>
                  <th colspan="11" class="border border-[#E8E8E8] bg-[#FFF3ED] text-[#FF7B22] px-2 py-1 text-center font-semibold">
                    N년이내 입원/수술
                  </th>
                  <th rowspan="2" class="border border-[#E8E8E8] bg-[#FFE4EC] text-[#E91E63] px-2 py-1.5 text-center font-semibold whitespace-nowrap align-middle">
                    N년이내<br/>중대질환
                  </th>
                </tr>
                <tr>
                  <th v-for="n in 11" :key="n" class="border border-[#E8E8E8] bg-[#FFF8F3] text-[#999] px-1 py-1 text-center font-medium min-w-[24px]">
                    {{ n - 1 }}
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td class="border border-[#E8E8E8] px-2 py-2 text-center font-semibold text-[13px]"
                  ...
                  <td v-for="n in 11" :key="n" class="border border-[#E8E8E8] px-1 py-2 text-center text-[12px]"
                    :class="getHospSurgeryTotal(n - 1) > 0 ? 'text-[#333] font-semibold bg-[#FFF8F3]' : 'text-[#CCC]'">
                    {{ getHospSurgeryTotal(n - 1) || '' }}
                  </td>
                  <td class="border border-[#E8E8E8] px-2 py-2 text-center font-semibold text-[12px]"
          ...
          <p class="text-[10px] text-red-400 mt-2">* '3개월 이내'는 입원/수술일 기준으로 산정됩니다.</p>
          <p class="text-[10px] text-[#BBB] mt-0.5">※ 상세 보장 내용은 약관을 참고해 주세요.</p>

          <!-- 중대질환 상세 -->
          <div v-if="hasCriticalDisease" class="mt-3 pt-3 border-t border-[#F0F0F0]">
            <p class="text-[12px] font-semibold text-[#E91E63] mb-2">중대질환 해당 항목</p>
            <div class="flex flex-wrap gap-1.5">
              <span v-for="(info, key) in store.disclosureCheck.critical_diseases" :key="key"
                ...
                class="px-2 py-1 text-[11px] rounded-full border font-medium">
        ...
        <div v-else-if="store.disclosureCheckLoading" class="bg-white rounded-[14px] border border-[#E8E8E8] p-6 mb-3 text-center">
          <div class="animate-spin rounded-full h-6 w-6 border-b-2 border-[#FF7B22] mx-auto" />
          <p class="text-[12px] text-[#999] mt-2">알릴의무 체크 로딩 중...</p>
        </div>

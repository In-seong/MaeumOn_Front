...
const agentName = computed(() => authStore.agent?.agent_name ?? '설계사')
const agencyName = computed(() => authStore.agent?.agency_name ?? '소속 없음')
const agentPosition = computed(() => authStore.agent?.position ?? '')
const agentPhone = computed(() => authStore.agent?.agent_phone ?? '-')
const agentEmail = computed(() => authStore.agent?.agent_email ?? '-')
